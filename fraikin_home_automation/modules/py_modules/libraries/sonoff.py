...
class Sonoff():
    ...
    def do_login(self):
        ...
        decryptedAppSecret = b'mXLOjea0woSMvK9gw7Fjsy7YlFO4iSu6'
    ...
    def _get_ws(self):
        ...
        if self._ws is None:
            try:
                ...
                ts = time.time()
                payload = { 'action': 'userOnline', 'at': self.get_bearer_token(),  'apikey': self.get_user_apikey(), 'userAgent': 'app', 'appid': self.appid, 'nonce': str(int(ts / 100)), 'ts': int(ts), 'version': 8, 'sequence': str(int(ts * 1000)) }
#                payload = {
#                    'action'    : "userOnline",
#                    'userAgent' : 'app',
#                    'version'   : 8,
