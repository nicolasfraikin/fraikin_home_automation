...
def get_is_enum(type, enum_definitions):
    return type in enum_definitions


def convert_cpp_to_java_default_value(cpp_default_value, type=None):
    ret = (CPP_TO_JAVA_DEFAULT_VALUE[
               cpp_default_value] if cpp_default_value in CPP_TO_JAVA_DEFAULT_VALUE.keys() else cpp_default_value).replace(
        "::", ".")
    if type is not None and "string" in type:
        ret = '"' + ret + '"'
    return ret
...
def create_app_interface(output_file, template_path, interface_name, interface_short_name, interface_data_type):
    ...
    enum_names = [member["EnumName"] for member in data_type_definition["EnumDefinitions"]]
    res["EnumNames"] = enum_names
    res["EnumDefinitions"] = zip(enum_names,
    ...
    res["DataTypeMembers"] = zip(
        ...
        [convert_cpp_to_java_default_value(member["DefaultValue"], member["Type"]) for member in
         data_type_definition["DataTypeMembers"]])
    res["DataTypeMembers2"] = zip(
        [convert_cpp_type_to_java_type(member["Type"]) for member in data_type_definition["DataTypeMembers"]],
        [member["VariableName"] + "_in" + (
    ...
    res["DataTypeMembers4"] = zip(
        [get_is_enum(member["Type"], enum_names) for member in
         data_type_definition["DataTypeMembers"]],
        [member["VariableName"] for member in data_type_definition["DataTypeMembers"]],
        [convert_cpp_to_java_default_value(member["DefaultValue"]) for member in
         data_type_definition["DataTypeMembers"]])
