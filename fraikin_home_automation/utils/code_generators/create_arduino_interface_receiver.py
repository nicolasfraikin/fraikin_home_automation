...
def create_arduino_interface_receiver(output_file, template_path, interface_list, interface_data_type_list):
    ...
    res["InterfaceNamesPascalCase"] = [pascal_case(interface) for interface in interface_list]
    ...
    res["InterfaceNamesAndDataTypes"] = zip(res["InterfaceNamesPascalCase"], interface_data_type_assignments)
...
if __name__ == "__main__":
    ...
    interface_data_type_list = sys.argv[4].split(",")
    interface_list = [item for item in interface_list if len(item) > 0]
    print("##### " + str(interface_list))

    create_arduino_interface_receiver(output_file, template_path, interface_list, interface_data_type_list)
