GENERATOR = "//fraikin_home_automation/utils/code_generators:create_arduino_interface_sender"
TEMPLATE_PATH_H = "templates/arduino_interface_sender_h.jinja"
TEMPLATE_PATH_CPP = "templates/arduino_interface_sender_cpp.jinja"

def _create_arduino_interface_sender_impl(ctx):
    # Define output files
    ...
    ctx.actions.run(
        ...
        arguments = [output_file.path, template_path, ctx.attr.interface_list],
    )
...
create_arduino_interface_sender = rule(
    _create_arduino_interface_sender_impl,
